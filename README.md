...
```
docker build -t my-siridb-http:latest -f docker/Dockerfile .
```

#### Supplying full config file

Overried the config file generated by docker with the `SIRIDB_HTTP_CONFIG`
environment var:

```
docker run --name siridb-http \
    -e SIRIDB-HTTP_CONFIG=/etc/siridb-http.conf
    -v /my/local/siridb-http.conf:/etc/siridb-http.conf:ro
    siridb-http:latest
```
